				...
				<div class="row justify-content-center">
					<div class="col-lg-10 text-center">
						<h3 class="text-uppercase font_black font-weight-bold">FILL
							THE FORM TO UPDATE AN EXISTING RECYCLING IDEA</h3>
						<hr class="divider border_light my-4" />
					</div>
					<div class="col-lg-8 align-self-start">
						<form action="/gaia/FrontController.do" method="post">
							<div class="form-group text-center">
								<label class="font_black" for="inputCod">Identifier
									code: ${recyclingIdeasCd}</label> <input type="hidden" name="code"
									value="${recyclingIdeasCd}">
							</div>
							<div class="form-row">
								...
								<div class="form-group col-md-4 text-center">
									<label class="font_black" for="inputMaterial">Main
										...
										<c:forEach items="${allRecycledMaterial}" var="recycled">
											<option value="${recycled.name}"
												${recycled.name == recyclingIdeasMaterial ? "selected":""}>${recycled.name}</option>
										</c:forEach>
									</select>
								</div>
							</div>
